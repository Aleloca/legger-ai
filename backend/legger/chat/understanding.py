...
#: path, so a short timeout + a single retry beats the SDK default (10 min).
...
def _format_input(messages: list[dict]) -> str:
    ...
    return (
        f"Storico della conversazione (turni più recenti):\n{transcript}\n\n"
        f"Messaggio corrente dell'utente:\n{messages[current_idx]['content']}"
    )
...
def understand_query(messages: list[dict], *, anthropic_client: Anthropic) -> QueryAnalysis:
    ...
    """
    fallback = QueryAnalysis(rewritten_query=last_user_message(messages))
    try:
